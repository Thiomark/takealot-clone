    ...
    return (
        <div>
            <div className='mx-auto max-w-[1600px]'>
                <div className='flex xl:px-20 px-4 py-2 items-center justify-between'>
                    <h1 className='capitalize tet-sm'>{title}</h1>
                    <button className='px-4 py-2 text-sm border-[1.5px] border-gray-400'>View More</button>
                </div>
                <div className='xl:mx-20 mx-4 py-2 gap-4 relative items-stretch group flex overflow-hidden'>
                    ...
                    {[...Array(5)].map((product, index) => <ProductComponent key={index}/>)}
                </div>
            </div>
        </div>
    )
