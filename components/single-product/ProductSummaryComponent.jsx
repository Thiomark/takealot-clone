...
const ProductSummaryComponent = ({ style }) => {
  ...
  return (
    <div className={style}>
      <h1 className="font-semkbold hidden xl:block mt-4 lg:mt-0 text-2xl">
        {product.name}
      </h1>
      <RatingComponent
        reviews={product.reviews}
        rating={product.rating}
        extraStyle="hidden xl:flex xl:py-5"
      />
      <div className="lg:border-y border-gray-200 py-2">
        <div className="">
          <h1 className="font-bold items-center">
            In stock{" "}
            <span className="text-xs p-1 font-normal mx-2 rounded bg-gray-200">
              JHB
            </span>
            <span className="font-normal text-sm text-blue-450 cursor-pointer hover:underline">
              When do I get it?
            </span>
          </h1>
        </div>
      ...
      <ul className="list-none text-xs divide-y xl:divide-none divide-dashed divide-gray-300">
        {items.map((item, index) => (
          <li
            className="py-2.5 flex items-center justify-between xl:justify-start"
            key={index}
          >
            <div className="flex items-center">
              <svg
                ...
                fill="currentColor"
                className="bi w-4 text-gray-700 h-4 bi-info-circle-fill"
                viewBox="0 0 16 16"
              >
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              className="bi xl:ml-2 w-3 text-green-450 h-3 bi-info-circle-fill"
              viewBox="0 0 16 16"
            >
