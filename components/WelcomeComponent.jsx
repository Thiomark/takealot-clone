                    ...
                    <div className="max-w-5xl mx-auto py-3 hidden lg:space-x-4 md:block lg:flex px-4">
                        ...
                        <div className="space-y-3 w-full">
                            ...
                            <div className="flex divide-x-[1px] divide-gray-300">
                                {buttons.map((button, index) => <button className={`${index === 0 ? 'rounded-l-md' : ''} ${index === buttons.length - 1 ? 'rounded-r-md' : ''} text-xs flex-shrink flex-grow px-3 py-2 bg-gray-250`} key={button}>{button}</button>)}
                            </div>
                        </div>
                    </div>
                </div>
                <div className="hidden h-80 md:block lg:hidden">
                    <img className="w-full h-full object-cover" src="https://images.pexels.com/photos/5887647/pexels-photo-5887647.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2" alt="" />
                </div>
                <div className="lg:grid hidden grid-cols-4 mx-auto max-w-5xl p-4 space-x-2">
                    <div className="col-span-3 h-80 bg-gray-300 lg:ml-[215px]">
                        {/* <CarousalComponent /> */}
                    </div>
                    <div className="bg-blue-100 hidden lg:flex shadow-md border-2 border-white items-center overflow-hidden h-20">
                        <div className="bg-blue-250 rounded-full -ml-14 overflow-hidden flex items-center justify-center h-32 w-32">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi w-16 h-16 bi-truck" viewBox="0 0 16 16">
