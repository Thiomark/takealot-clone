...
const ProductComponent = ({showAddToCart}) => {
    ...
    const { fetchProduct, addItemToCart } = useContext(ProductContext);
    ...
    return (
        <div className='md:min-w-[280px] min-w-[68%] w-full cursor-pointer bg-white p-4' onClick={() => {
            ...
            <div className='md:h-64 h-52 justify-center items-center relative bg-gray-200 mb-4'>
                {product.oldPrice > product.price && <span className="bg-blue-450 p-2 absolute top-0 left-0 font-bold text-xs text-white h-12 w-12 z-20 rounded text-center flex items-center">{product.sale} % off</span>}
                <img className="w-full absolute h-full" src={`placeholder-images/${product.displayedImage}-placeholder.png`} alt={product.name} srcSet="" />
            </div>
            <div className="h-14">
                <p className='text-sm text-gray-600 pb-4'>{product.name}</p>
            </div>
            <h1 className='text-sm font-bold mb-1'>R {product.price} {product.oldPrice > product.price && <span className='text-xs text-gray-400 font-normal line-through ml-2'>R {product.oldPrice}</span>}</h1>
