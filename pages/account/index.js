      ...
      <div className="md:p-4 md:sides xl:px-20 w-full">
        ...
        <div className="pb-10 md:grid grid-cols-2 lg:grid-cols-3 gap-6 grid-rows-[repeat(3,minmax(13em,1fr))]">
          {sections.map((section, index) => (
            <div
              key={index}
              className={`md:bg-white md:shadow md:py-4 ${section.HideOnMobile && "hidden md:block"}`}
            ...
            >
              <header className="md:pb-2 py-4 md:py-0 font-semibold text-gray-900 px-5 md:px-4 md:flex items-center justify-between">
                <h1 className="">
                  <span className={`${section.secondName && "md:hidden"}`}>
                    {section.name}
                  </span>
                  <span
                    className={`${section.secondName && "hidden md:block"}`}
                  >
                    {section.secondName}
                  </span>
                </h1>
                <svg
                  ...
                  fill="currentColor"
                  className="bi h-5 hidden md:block w-5 bi-cart-fill"
                  viewBox="0 0 16 16"
                >
                  {section.svg()}
                </svg>
